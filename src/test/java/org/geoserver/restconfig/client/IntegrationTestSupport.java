...
/**
 * Junit {@link Rule} used to provide a pre-configured {@link GeoServerClient}
 * and to enable integration tests only if the geoserver instance is
 * {@link #isAlive() accessible}
 * <p>
 * The {@code geoserver_port} environment variable needs to be provided
 */
@Slf4j
public class IntegrationTestSupport extends ExternalResource {
	...
	protected @Override void before() throws IOException {
		...
		final String publicUrl = resolve("geoserver_public_url", "http://carto.example.com/geoserver");
	...
	public boolean isAlive() {
		if (null != this.client) {
			...
			try {
				...
				log.info("GeoServer unavailable at URL {}: {}", this.client.api().getBasePath(), e.getMessage(), e);
			}
		}
		return false;
	}
	...
	public WorkspaceInfo createWorkspace(@NonNull String name, boolean isolated) {
		...
		WorkspaceInfo ws = workspaces.getAsInfo(name)
