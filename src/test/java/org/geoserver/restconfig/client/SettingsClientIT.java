...
public class SettingsClientIT {
	...
	public @Test void createAndChangeWorkspaceSettings() {
		...
		ContactInfo contact = new ContactInfo().address("address").addressCity("addressCity")
				.addressCountry("addressCountry").addressType("addressType").contactEmail("contact@example.com");

		SettingsInfo request = new SettingsInfo();
		request.charset("ISO-8859-1").contact(contact).onlineResource("http://example.com").title("contact title")
				.contact(contact);
