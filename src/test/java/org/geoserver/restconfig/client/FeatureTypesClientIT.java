...
public class FeatureTypesClientIT {
	...
	public @Test void createRequiresName() {
		FeatureTypeInfo roads = new FeatureTypeInfo().nativeName("roads")
...
//		ex.expectMessage("No such data store");
	...
	/**
	 * It should be possible to create two feature types with the same store and FT
	 * name on different workspaces. Fails with geoserver < 2.15.4, bug report:
	...
	public @Test void createSameStoreNameDifferentWorkspace() {
		...
		FeatureTypeInfo streamsWorkspace1 = new FeatureTypeInfo().nativeName(nativeFeatureTypeName)
				.store(new DataStoreInfo().name(storeName));
		FeatureTypeInfo streamsWorkspace2 = new FeatureTypeInfo().nativeName(nativeFeatureTypeName)
				.store(new DataStoreInfo().name(storeName));
		{
		...
		{
			FeatureTypeInfo ft2ws1 = this.featureTyes.create(ws1.getName(), streamsWorkspace1.name("name2"));
		...
		{
			FeatureTypeInfo ft2ws2 = this.featureTyes.create(ws2.getName(), streamsWorkspace2.name("name2"));
	...
	public @Test void createMinimalInformation() {
		FeatureTypeInfo roadsws1 = new FeatureTypeInfo().name("roads").nativeName("roads")
				.store(new DataStoreInfo().name("roadsStoreWs1"));
		FeatureTypeInfo roadsws2 = new FeatureTypeInfo().name("roads").nativeName("roads")
	...
	public @Test void createMinimalInformationNativeAndPublishedName() {
		FeatureTypeInfo roadsws1 = new FeatureTypeInfo().nativeName("roads").name("roads-123")
				.store(new DataStoreInfo().name("roadsStoreWs1"));
		FeatureTypeInfo roadsws2 = new FeatureTypeInfo().nativeName("roads").name("roads-345")
				.store(new DataStoreInfo().name("roadsStoreWs2"));
