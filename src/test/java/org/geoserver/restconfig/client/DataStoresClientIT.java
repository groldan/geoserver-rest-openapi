...
public class DataStoresClientIT {
	...
	public @Test void testGet() throws MalformedURLException {
		...
		assertThat(store.getFeatureTypes().toURL().toString(),
	...
	public @Test void testCreateShapefileDataStoreDefaults() {
		DataStoreResponse created = this.dataStores.createShapefileDataStore(this.workspace.getName(),
				"shpstore_defaults", this.streamsShapefile);
	...
	public @Test void testCreateShapefileDataStore() {
		...
		Shapefile connectionParameters = new DataStoreParams.Shapefile().uri(uri).useMemoryMappedBuffer(true)
				.useMemoryMaps(true).createSpatialIndex(true);
		String wsname = this.workspace.getName();
		DataStoreResponse created = this.dataStores.create(wsname, "shpStore", "test shapefile based data store",
	...
	public @Test void testCreateShapefileDataStoreDuplicateName() {
		...
		DataStoreResponse nonDuplicateNameSameUri = this.dataStores.createShapefileDataStore(wsname,
				"nonDuplicateNameSameUri", uri);
	...
	public @Test void testUpdateShapefileDataStore() {
		...
		assertNull(
				this.dataStores.toConnectionParameters(created.getConnectionParameters()).get("memory mapped buffer"));
