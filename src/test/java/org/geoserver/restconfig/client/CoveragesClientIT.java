...
import org.geoserver.restconfig.api.v1.model.CoverageStoreResponse;
...
import org.geoserver.restconfig.model.catalog.CoverageStoreInfo.TypeEnum;
import org.geoserver.restconfig.model.catalog.ProjectionPolicy;
...
public class CoveragesClientIT {
	...
	public @Before void before() {
		...
		this.sfdemStore = this.coverageStores.create(wsname1, "sfdem", "Test geotiff", TypeEnum.GEOTIFF,
				sfdemURI.toString());
	}
	...
	/**
	 * This is a bug in geoserver, providing anything other than a simple name makes
	 * it ignore the provided "published name" and not automatically compute other
	 * fields, but uses exactly what was provided (it does compute the bounds and
	 * native CRS though)
	 */
	public @Test void createWithTitleAndAbstractDoesNotAtuoComputeSomeFields() {
		CoverageInfo sfdem = new CoverageInfo().nativeCoverageName("sfdem").name("PublishedName").title("title")
				._abstract("abstract").description("description")
				.store(new CoverageStoreInfo().name(sfdemStore.getName()));
		sfdem.setName(null);
		CoverageInfo created = this.coverages.create(ws1.getName(), sfdem);
		assertNotNull(created);
		...
		assertEquals("EPSG:26713", created.getSrs());
		assertThat(created.getNativeCRS(), StringStartsWith.startsWith("PROJCS[\"NAD27 / UTM zone 13N\""));
		assertEquals("Should have been REPROJECT_TO_DECLARED", ProjectionPolicy.FORCE_DECLARED,
	...
	/**
	 * It should be possible to create two coverages with the same store and native
	 * coverage name on different workspaces. This currently fails, although it was
	 * fixed for FeatureTypes with geoserver >= 2.15.4,bug report:
	...
	public @Test void createSameStoreNameDifferentWorkspace() {
		...
		CoverageInfo requestBody = new CoverageInfo().nativeCoverageName(coverageName)
		...
		CoverageStoreResponse store2 = this.coverageStores.create(workspace2, storeName, store1.getDescription(),
				TypeEnum.GEOTIFF, sfdemURI.toString());
	...
	/**
	 * Due to a bug in some geoserver versions, can't create a coverage with a name
	 * that already exists in another workspace/coveragestore. Nonetheless, we'll
	 * make sure to provide unique coverage names, but creating coverages for the
	 * same geotiff file in different workspaces, re-using the each workspace
	 * coveragestore, should be possible
	 */
	@Ignore
	public @Test void createSameStoreNameDifferentWorkspaceUniqueCoverageName() {
		...
		// create a store with the same name than store1
		CoverageStoreResponse store2 = this.coverageStores.create(workspace2, storeName2, store1.getDescription(),
				TypeEnum.GEOTIFF, sfdemURI.toString());
		assertNotNull(store2);

		CoverageInfo requestBodyWs1 = new CoverageInfo().nativeCoverageName(nativeCoverageName)//
				.store(new CoverageStoreInfo().name(storeName1))//
				.name("sfdem-ws1");
		CoverageInfo requestBodyWs2 = new CoverageInfo().nativeCoverageName(nativeCoverageName)//
				.store(new CoverageStoreInfo().name(storeName1))//
				.name("sfdem-ws2");
		{
	...
	public @Test void updateBasicMetadata_UseNewCoverageInfo() {
		...
		CoverageInfo updateRequestBody = new CoverageInfo().name("UpdatedName").title("Updated Title")
