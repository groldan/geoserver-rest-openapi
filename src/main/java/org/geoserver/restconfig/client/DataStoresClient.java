...
import org.geoserver.restconfig.api.v1.client.DatastoresApi;
...
import org.geoserver.restconfig.api.v1.model.DataStoreInfoWrapper;
import org.geoserver.restconfig.api.v1.model.DataStoreListWrapper;
...
public class DataStoresClient {
	...
	public DataStoreResponse create(@NonNull String workspace, @NonNull String name, String description,
	...
	public DataStoreResponse createShapefileDataStore(@NonNull String workspace, @NonNull String storeName,
			@NonNull URI shpfile) {
	...
	public List<Link> findByWorkspace(@NonNull String workspace) {
		DataStoresListResponse wrapper = this.client.collectionCall(() -> this.api().getDatastores(workspace),
				() -> new DataStoresListResponse().dataStores(new DataStoreListWrapper()));
		return Link.map(wrapper.getDataStores().getDataStore());
	}

	public Optional<DataStoreResponse> findByWorkspaceAndName(@NonNull String workspaceName,
	...
	public ConnectionParameters toConnectionParameters(@NonNull Map<String, String> params) {
		return new ConnectionParameters()
				.entry(params.entrySet().stream().map(this::connectionParameterEntry).collect(Collectors.toList()));
	}
