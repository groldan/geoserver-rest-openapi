...
/**
 * Feign {@link ErrorDecoder} that returns application specific
 * {@link ServerException} instead of {@link FeignException}
 */
public class GeoServerFeignErrorDecoder implements ErrorDecoder {
	...
	public @Override ServerException decode(String methodKey, Response response) {
		...
		if (headers.containsKey("Content-Type") && headers.get("Content-Type").contains("text/plain")) {
			try {
				String errorText = new BufferedReader(response.body().asReader()).lines()
