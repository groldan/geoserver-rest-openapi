...
import org.geoserver.restconfig.model.catalog.StyleInfo.FormatEnum;
...
public class StylesClient {
	...
	public StyleList getStyles(@NonNull String workspaceName) {
		return client.collectionCall(() -> api().getStylesByWorkspace(workspaceName).getStyles(), StyleList::new);
	}
	...
	public String getBody(@NonNull StyleInfo style) {
		...
		case MBSTYLE:
			if (null == style.getWorkspace())
				...
				return getBody(style.getWorkspace().getName(), style.getName(), StyleFormat.MAPBOX);
		case SLD:
			if (null == style.getWorkspace())
				return getBody(style.getName(), StyleFormat.SLD_1_0_0);
			else
				return getBody(style.getWorkspace().getName(), style.getName(), StyleFormat.SLD_1_0_0);
		default:
			throw new UnsupportedOperationException("Unknown or unsupported style output format: " + style.getFormat());
		}
	}
	...
	/**
	 * Creating a new style is a two step process, first the {@link StyleInfo} is
	 * created through {@link StylesApi#createGlobalStyle} or
	 * {@link StylesApi#createStyle}, and then the actual style document is uploaded
	 * through {@link StylesApi#putStyle}.
	 * <p>
	 * This method makes sure to roll back the {@code StyleInfo} creation in case
	 * the upload fails.
	 */
	...
	public StyleInfo createMapboxStyle(@NonNull String workspaceName, @NonNull String name,
	...
	public StyleInfo createStyle(@NonNull StyleInfo info, @NonNull String requestBody) {
		...
		case MBSTYLE:
			if (null == info.getWorkspace()) {
				...
				api.uploadStyleMapbox(info.getWorkspace().getName(), info.getName(), requestBody.getBytes(UTF_8),
						false);
			}
			break;
		case SLD:
			if (null == info.getWorkspace()) {
				...
				api.uploadStyleSLD(info.getWorkspace().getName(), info.getName(), requestBody.getBytes(UTF_8), false);
			}
			break;
		default:
			throw new UnsupportedOperationException("Unknown or unsupported style output format: " + info.getFormat());
		}
		return info;
	}
		...
		if (!atLayers.isMissingNode()) {
			...
			if (layers.size() > 1) {
				layers.forEach(layerNode -> {
					...
					if (!(sourceLayer instanceof TextNode) || ((TextNode) sourceLayer).asText().isEmpty()) {
	...
	/**
	 * Deletes a style; the accompanying style body file is deleted from the
	...
	/**
	 * Deletes a style from a workspace; the accompanying style body file is deleted
	 * from the filesystem and the style reference removed from any layer that
	 * points to it
	 * 
	 ...
	 * @param deleteFile Specifies whether the underlying file containing the style
